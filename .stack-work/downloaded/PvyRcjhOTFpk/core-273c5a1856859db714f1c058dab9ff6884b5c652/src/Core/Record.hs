...
-- |
-- Infix version of 'get', with the arguments flipped. E.g. @record \- #name@.
-- 
(\-)
  :: Has label value record
  => record
...
-- |
-- @setIf #allowed False (any (== Denied) es)@
setIf :: Has label value record => Proxy label -> value -> Bool -> record -> record
setIf l v toggle rec = if toggle
  then set l v rec
  else rec


setJust :: (Has l v r, v ~ Maybe a, KnownSymbol l) => Proxy l -> a -> r -> r
setJust l x r = set l (Just x) r
